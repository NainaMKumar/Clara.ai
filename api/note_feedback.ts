...
function toFeedbackItemsShape(input: unknown): NoteFeedbackItem[] {
  ...
  for (const it of itemsRaw) {
    ...
    if (kind !== 'missing' && kind !== 'inaccurate' && kind !== 'specificity')
      continue;
...
async function tavilySearch(args: {
  ...
  if (!resp.ok) {
    const body = await resp.text().catch(() => '');
    throw new Error(
      `Tavily search error: ${resp.status} ${resp.statusText} ${body}`
    );
  }
  ...
  return results
    ...
    .filter(
      (x): x is { title: string; url: string; content: string } => x !== null
    )
    .slice(0, 6);
}
