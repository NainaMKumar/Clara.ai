import type { NoteFeedbackFixRequest, NoteFeedbackFixResponse, NoteFeedbackItem } from './_types'
import { env } from './env'
...
function safeItem(input: any): NoteFeedbackItem {
  ...
  return {
    kind: kind === 'missing' || kind === 'inaccurate' || kind === 'specificity' ? kind : 'specificity',
    quote,
    issue,
    suggestion,
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const ctx = (body as any).context as
      | { containerType?: string; listType?: 'bulletList' | 'orderedList'; blockText?: string }
      | undefined
    ...
    if (!item.quote?.trim()) return sendJson(res, 400, { error: 'item.quote is required' })
    if (!item.issue?.trim()) return sendJson(res, 400, { error: 'item.issue is required' })
    ...
    const contextBlock = [
      ...
      ctx?.blockText ? `blockText: ${clampText(ctx.blockText, 900)}` : 'blockText: (none)',
    ].join('\n')
    ...
    const mode = toMode((parsed as any)?.mode ?? (preferredMode === 'replace' ? 'replace' : 'insert'))
